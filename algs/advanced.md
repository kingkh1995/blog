...
图的生成树是它的一棵含有所有顶点的无环连通子图，而加权无向图的最小生成图是它的一棵权值之和最小的生成树。
...
根据**切分定理**，找到一种切分，它的横切边均不为黑色，则将它的权重的横切边标记为黑色，重复，直到标记了V-1条边为止。
...
每一步都为一棵生长中的树添加一条边，一开始这颗树只有一个顶点，每次都将下一条连接树中的顶点与不在树中的顶点且权重最小的边加入树中，直到添加V-1条边。
