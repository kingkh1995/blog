...
都是多线程的新生代垃圾收集器，Parallel Scavenge 追求 CPU 吞吐量，能够在较短时间内完成指定任务；ParNew 追求降低用户停顿时间。

***

## CMS

老年代垃圾收集器，并发收集，以最短的停顿时间作为目标。

对CPU资源有一定占用，吞吐量低；使用标记-清除算法产生空间碎片，故会引发Full GC，重新整理内存。
...
- Mixed GC：同时回收新生代加老年代区域，【-XX:InitiatingHeapOccupancyPercent】配置表示当老年代占用比例超过该值后，执行Mixed GC替代Young GC，默认45%；Mixed GC还会回收经过并发标记后统计后，确定的回收益最高的老年代区域；
