...
Parallel Scavenge的老年代版本，并发收集，基于标记-整理算法，与Parallel Scavenge配合使用，适用于“吞吐量优先”场景。
...
**染色指针是一种将少量额外信息存储在指针上的技术**。64位系统中实际上指针只有低48位可以用来寻址，ZGC将这其中的高4位用于存储四个标志信息，即一个指针只有44位的地址空间，故ZGC可管理的最大内存为16TB，且不再支持指针压缩。由于染色指针重新定义了指针，需要使用**虚拟内存映射技术**去支持正常的寻址操作，使用多重映射的方式，将多个虚拟内存地址（一个染色指针不同的视图对应到不同的虚拟地址）映射到同一个物理内存地址上。
