...
使用解释和编译混合执行的模式，在启动时JVM先将字节码解释执行，省去了编译时间，屏蔽了对底层操作系统的依赖，随着时间推进，通过热点代码统计分析，使用JIT技术动态的将热点代码编译为机器码直接交由CPU执行，以提高执行效率。
...
线程私有，每一个运行的Java方法都有一块自己的栈帧，用于存放该方法运行过程中的一些信息，包括局部变量表、操作数栈、动态链接、方法出口信息等。
...
JDK8中使用元空间替代了永久区，而字符串常量池也被移动到了堆内存，元空间存放以下信息：已经被虚拟机加载的类信息、常量、静态变量、即时编译器编译后的代码。
...
- 对象头：包括markword和class对象指针。markword包含哈希码、GC 分代年龄、锁状态标志，偏向锁标识（已废弃），如果是数组还包括数组长度，在32位JVM中是4字节，64位JVM则是8字节；class对象指针大小为4字节，指向类元数据的首地址。
- 实例数据：成员变量的值，包括所有父类可见的和自身定义的。
