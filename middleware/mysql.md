...
- B树索引：
    - 使用B+树实现，有序存储，一般为1-3层（主键为8字节的bigint时3层的B+树可以存储2000多万条数据），非叶子结点只保存索引值，在叶子结点才保存被索引的数据，同时叶子结点之间通过指针顺序连接。
...
- 尽量不要使用包含NULL的列，因为优化器难以优化，可能会导致索引失效；
- 尽量不要选择被频繁更新的列，因为索引维护成功高；
- 尽量考虑联合索引，避免索引冗余；
- 字符串列尽量使用前缀索引。
