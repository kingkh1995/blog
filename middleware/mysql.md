...
- 多版本并发控制协议，innodb引擎的实现会在每一行数据中额外保存，数据版本号 db_trx_id（最后更新该数据的事务id）和回滚指针 db_roll_pt（指向undolog的指针），删除位（标识是否被删除）。
...
#### innodb mvcc 更新过程
获取排他锁-修改该行-写redolog-写undolog-修改当前版本号，将回滚指针指向undolog历史数据   

#### readview
> 配合MVCC使用，组成：当前进行中的事务Id集合，当前事务ID，执行事务Id集合中的最小值，当前将要分配的下一个事务Id（最大事务ID）。
...
- 取到版本链最新版本，首先判断该版本记录的trx_id与ReadView中的creator_trx_id是否相等，不相等则进行下面步骤。
  ...
  - 3. 在中间，如果是RR级别则不可读，如果是RC级别，则在进行的事务集合中二分查找，判断该事务是否提交，未提交则不可见。
...
- 记录锁：锁住索引而不是记录本身，更新或删除数据需要获取排他记录锁，排他记录锁会被所有记录锁阻塞，共享记录锁只会被排他记录锁阻塞。
...
- B树索引
    - 使用B+树实现，有序存储，一般为1-3层（主键为8字节的bigint时3层的B+树可以存储2000多万条数据），非叶子结点只保存索引值，在叶子结点才保存被索引的数据，叶子结点之间通过指针顺序连接。
...
#### 索引下推
Mysql5.6的版本上推出，用于优化查询，存在某些被索引的列的判断条件时，MySQL服务器将这一部分判断条件传递给存储引擎，然后由存储引擎通过判断索引是否符合MySQL服务器传递的条件，只有当索引符合条件时才会将数据检索出来返回给MySQL服务器。
