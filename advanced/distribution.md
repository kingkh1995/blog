...
- **消息最终一致性**：发送消息成功后提交本地事务，优点是削峰填谷，缺点是要求消息组件可靠性、不满足一致性、无法回滚；
...
- 步骤：
  ...
  3. Acceptor开始表决，Acceptor无条件同意从未接收过的提案，不同意比自己以前接收过的提案编号要小的提案，一旦提案多数同意后进入下一步；
  4. 编号为N的提案通过，Learner记录提案，并将结果返回客户端。
