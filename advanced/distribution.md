...
分为两种思想TCC和Saga，TCC实现复杂，需要三个操作try、confirm和cancel，try之后数据处于软状态，能保证一致性；Saga实现简单，只需要实现执行模块和补偿模块即可，补偿模块分为向前补偿和向后补偿，如果是向后补偿则可以直接等同于执行模块，因为分支事务可能处于部分成功和部分失败的状态，故不满足一致性；**柔性事务只能实现最终一致性，其关键就是保证操作的幂等**。

- **定时补偿机制**：持久化请求信息到数据库后返回，先尝试异步处理一次保证时效性，然后定时补偿直到成功，**可靠性高但实现复杂**。
- **消息最终一致性**：使用事务消息或本地消息表的方式保证消息能被成功发送，再保证消费者消费成功即可，**缺点是要求消息中间件是可靠的、无法回滚、过程中不满足一致性**。
