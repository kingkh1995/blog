...
可能发生在生产者端、MQ端、消费者端，总结来说是生产者保证至少成功发送一次，MQ保证消息落盘后才返回成功，MQ保证消费者至少成功消费一次。

### 生产者丢消息

应该选择有回调方式，即不选择单向发送，同步发送不会出现丢消息问题（但会带来其他问题），**在严格的场景下应该使用事务消息（时效性严格场景）或本地消息表方案**。
...
- 消费消息时从业务角度判断当前消息是否是有效，**较难实现**；
- 去MQ端查询消息是否发送成功再决定后续操作，**场景太理想难以实现**；
- **发送端通过重试保证至少发送一次，消费端通过幂等保证最多消费一次**。
...
顺序消息分为局部有序和全局有序，**前提是保证有序的发送，同时消费者端一般也需要顺序消费**。如果记录了消息消费的顺序，消费端可以并发消费消费，只记录下消息的内容，之后按照消息的消费顺序进行异步处理。
...
- Page Cache：通过mmap提高IO速度。
- 零拷贝：使用直接内存，让网卡直接读取文件数据。
- 并行：每个topic拆分为多个partition。
- 批量传输：支持批量操作，消息达到一定阈值才发送到broker。
